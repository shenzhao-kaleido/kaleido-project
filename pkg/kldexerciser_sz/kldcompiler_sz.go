...
func GenerateTypedArgs(abi abi.ABI, methodName string, strargs []string) ([]interface{}, error) {
	...
	//fmt.Println("sz test for args ", strargs, len(strargs), len(strargs[0]))
	...
	// fmt.Println("sz test for args", typedArgs)
...
func CompileContract(solidityFile, evmVersion, contractName, method string, args []string) (*CompiledSolidity, error) {
	...
	//fmt.Println("sz test input file", solidityFile)
	solcVer, err := compiler.SolidityVersion("solc")
	//fmt.Println("sz test solcver", solcVer)
	if err != nil {
	...
	//fmt.Println("sz test complied", compiled)
	if err != nil {
